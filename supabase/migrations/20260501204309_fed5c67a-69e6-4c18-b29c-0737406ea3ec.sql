...
do $$ begin
  create type public.fit_dimension as enum (
    'length','toebox_width','forefoot_width','midfoot','heel','drop','cushion'
  );
...
do $$ begin
  create type public.fit_rating as enum (
    'much_too_tight','slightly_tight','perfect','slightly_loose','much_too_loose'
  );
exception when duplicate_object then null; end $$;
...
create index if not exists user_shoes_user_id_idx on public.user_shoes (user_id, created_at desc);
create index if not exists user_shoes_shoe_id_idx on public.user_shoes (shoe_id);
alter table public.user_shoes enable row level security;

drop policy if exists user_shoes_select_own on public.user_shoes;
create policy user_shoes_select_own on public.user_shoes
  for select to authenticated using (auth.uid() = user_id);
drop policy if exists user_shoes_insert_own on public.user_shoes;
create policy user_shoes_insert_own on public.user_shoes
  for insert to authenticated with check (auth.uid() = user_id);
drop policy if exists user_shoes_update_own on public.user_shoes;
create policy user_shoes_update_own on public.user_shoes
  for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists user_shoes_delete_own on public.user_shoes;
create policy user_shoes_delete_own on public.user_shoes
...
create index if not exists user_shoe_fits_user_shoe_id_idx on public.user_shoe_fits (user_shoe_id);
...
create policy user_shoe_fits_select_own on public.user_shoe_fits
  for select to authenticated using (
    exists (select 1 from public.user_shoes us where us.id = user_shoe_fits.user_shoe_id and us.user_id = auth.uid())
  );
drop policy if exists user_shoe_fits_insert_own on public.user_shoe_fits;
create policy user_shoe_fits_insert_own on public.user_shoe_fits
  for insert to authenticated with check (
    exists (select 1 from public.user_shoes us where us.id = user_shoe_fits.user_shoe_id and us.user_id = auth.uid())
  );
drop policy if exists user_shoe_fits_update_own on public.user_shoe_fits;
create policy user_shoe_fits_update_own on public.user_shoe_fits
...
create policy user_shoe_fits_delete_own on public.user_shoe_fits
